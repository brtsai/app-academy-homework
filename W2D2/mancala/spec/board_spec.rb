...
describe Board do
  ...
  describe "#valid_move?" do
    ...
    it "raises 'Cup doesn't exist' error if the entered position is not on the board" do
      expect do
        board.valid_move?(15)
      end.to raise_error("Cup doesn't exist")
    end

    it "raises 'Cup is empty' error if the cup at the entered position is empty" do
      expect do
        board.valid_move?(0)
      end.to raise_error("Cup is empty")
    end
  end
