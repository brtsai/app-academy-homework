...
def pulp_fiction_actors
  ...
  Actor.select('actors.id', 'actors.name').joins('JOIN castings ON castings.actor_id = actors.id').joins('JOIN movies ON movies.id = castings.movie_id').where("movies.title = 'Pulp Fiction'")
end
...
def uma_movies
  ...
  Movie.select('movies.id', 'movies.title', 'movies.yr').joins('JOIN castings ON castings.movie_id = movies.id').joins('JOIN actors ON actors.id = castings.actor_id').where("actors.name = 'Uma Thurman'").order('movies.yr desc')
end
